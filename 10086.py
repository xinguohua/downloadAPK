...
def parse_link(url):
    ...
    for bt in soup.find_all("a", class_='sub_column_load'):
        try:
            queue_link.put('http://mm.10086.cn'+bt['href'])
        except:
            continue
...
if __name__ == '__main__':
    ...
    num = 1
    url='http://mm.10086.cn/android/software/qbrj?pay=1'
    type_url=[]
    sp = BeautifulSoup(link2content(url), 'html.parser')
    for ty in sp.find_all("a",class_='each_mx_noun'):
        type_url.append('http://mm.10086.cn'+ty['href'])
    for t in type_url:
        for page in range(1,8):
                ...
                while not queue_link.empty():
                    try:
                        ...
                        print(u)
                        wget.download(u , './apk/'+str(num)+'_10086.apk')#下载apk,参数为下载链接和路径
